...
function doReg(e) {
    ...
    //    构建POST请求的正文数据
        ...
        param += "&password=" + regpass;  // & 是分隔符
        param += "&ecode=" + regcode;
    //    利用Jquery框架发送post请求，并获取到后台注册接口的相应内容
        $.post('/user', param, function(data){
            if(data == "ecode-error"){
                bootbox.alert({title : "错误提示", message:"验证码无效。"});
                $("#regcode").val(''); //消除验证码框的值
...
$(document).ready(function () {
    $.get('/loginfo', function (data) {
        ...
        if(data == null) {
            ...
            content += '<a class="text-info bi bi-pencil-square nav-item nav-link" style="font-size:20px"  href="/ucenter/post">&nbsp;投稿</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
            content += '<a class="nav-item nav-link"  href="/ucenter-1">欢迎你：' + data['nickname']+'</a>&nbsp;&nbsp;&nbsp;';
            if (data['role'] == 'admin'){
                content += '<a class="nav-item nav-link"  href="/admin" >系统管理</a>&nbsp;&nbsp;&nbsp;';
            }
            else {
                content += '<a class="nav-item nav-link" href="/ucenter-1">用户中心</a>&nbsp;&nbsp;&nbsp';
            }
