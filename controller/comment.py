...
def add():
    ...
    if not comment.check_limit_per_5():
        try:
            ...
            # 评论成功后，更新积分明细和剩余积分
            Credit().insert_detail(types='添加评论', target=articleid, credit=2)
...
def reply():
    ...
    if not comment.check_limit_per_5():
        try:
            ...
            # 评论成功后，同步更新更新积分明细和剩余积分 回复数
            Credit().insert_detail(types='回复评论', target=articleid, credit=2)
            Users().update_credit(2)
            Article().update_replycount(articleid)
            return 'reply-pass'
        except:
...
# 为了使用Ajax分页，特创建此接口为演示
# 由于分页栏已经渲染完成，此接口根据前端的页码请求后台对应数据
